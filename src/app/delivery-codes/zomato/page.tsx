...
import { Truck, Copy, PlusCircle, CalendarDays, ArrowLeft, Heart } from "lucide-react";
...
export default function ZomatoCodesPage() {
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Zomato Codes.`,
    });
  };
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className={`text-3xl font-bold ${mode === 'gaming' ? 'font-orbitron' : ''}`}>
                  <a href="https://www.zomato.com" target="_blank" rel="noopener noreferrer" className="hover:underline">Zomato</a> Promo Codes
                </CardTitle>
              </div>
               <div className="flex items-center gap-2">
                {isDeveloperMode && (
                ...
                <Link href="/delivery-codes" passHref>
                  <Button
                    variant="outline"
                    className={cn(
          ...
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active Zomato codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
