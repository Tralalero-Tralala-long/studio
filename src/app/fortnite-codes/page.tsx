...
// Placeholder codes - can be populated by developer mode
...
export default function FortniteCodesPage() {
  ...
  const [codes, setCodes] = useState<PromoExample[]>(initialFortniteCodes);
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Fortnite Codes.`,
    });
  };
    ...
    <>
        ...
        <Card className={`${mode === 'gaming' ? 'bg-card border-primary' : 'bg-card'}`}>
          <CardHeader>
            ...
            <CardDescription className={`${mode === 'gaming' ? 'text-muted-foreground font-rajdhani' : 'text-muted-foreground'} pt-2`}>
              Active codes for Fortnite. Redeem them in-game for rewards!
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active Fortnite codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {isDeveloperMode && (
        <AddCodeForm
          ...
          formTitle="Add New Fortnite Code"
        />
      )}
    </>
  );
