...
export default function BlinkitCodesPage() {
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Blinkit Codes.`,
    });
  };
    ...
    <>
        ...
        <Card className={`${mode === 'gaming' ? 'bg-card border-primary' : 'bg-card'}`}>
          <CardHeader>
            ...
            <CardDescription className={`${mode === 'gaming' ? 'text-muted-foreground font-rajdhani' : 'text-muted-foreground'} pt-2`}>
              Active promo codes for Blinkit.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active Blinkit codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {isDeveloperMode && (
        <AddCodeForm
          ...
          formTitle="Add New Blinkit Code"
        />
      )}
    </>
  );
