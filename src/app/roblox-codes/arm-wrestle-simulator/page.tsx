...
export default function ArmWrestleSimulatorCodesPage() {
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Arm Wrestle Simulator.`,
    });
  };
    ...
    <>
        ...
        <Card className={`${mode === 'gaming' ? 'bg-card border-primary' : 'bg-card'}`}>
          <CardHeader>
            ...
            <CardDescription className={`${mode === 'gaming' ? 'text-muted-foreground font-rajdhani' : 'text-muted-foreground'} pt-2`}>
              Active codes for Arm Wrestle Simulator. Redeem them in-game for rewards!
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active Arm Wrestle Simulator codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {isDeveloperMode && (
        <AddCodeForm
          ...
          formTitle="Add New Arm Wrestle Simulator Code"
        />
      )}
    </>
  );
