...
export default function UdemyCodesPage() {
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Udemy Codes.`,
    });
  };
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className={`text-3xl font-bold ${mode === 'gaming' ? 'font-orbitron' : ''}`}>
                  <a href="https://www.udemy.com/" target="_blank" rel="noopener noreferrer" className="hover:underline">Udemy</a> Promo Codes
                </CardTitle>
              </div>
               <div className="flex items-center gap-2">
                {isDeveloperMode && (
          ...
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active Udemy codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
