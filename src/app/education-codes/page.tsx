...
import { School, Copy, PlusCircle, CalendarDays, ArrowLeft } from "lucide-react";
...
export const initialEducationCodes: PromoExample[] = [
  { id: "edu1", title: "50% Off Online Course", code: "LEARN50", expiry: "2025-01-31", platform: "Education", category: "online_course", description: "Get 50% off any online programming course.", isUsed: false },
  { id: "edu2", title: "Student Software Discount", code: "STUDENTPACK", expiry: "N/A", platform: "Education", category: "software_discount", description: "Access exclusive software discounts for students.", isUsed: false },
];

export default function EducationCodesPage() {
  const { mode, isDeveloperMode } = useAppContext();
  const { toast } = useToast();
  const [codes, setCodes] = useState<PromoExample[]>(
    initialEducationCodes
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Education Codes.`,
    });
  };
    ...
    <>
        ...
        <Card className={`${mode === 'gaming' ? 'bg-card border-primary' : 'bg-card'}`}>
          <CardHeader>
            ...
            <CardDescription className={`${mode === 'gaming' ? 'text-muted-foreground font-rajdhani' : 'text-muted-foreground'} pt-2`}>
              Find discounts for online courses, software, and educational resources.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {codes.length > 0 ? (
              ...
              <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
                No active education codes found at the moment. Check back soon!
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {isDeveloperMode && (
        <AddCodeForm
          ...
          formTitle="Add New Education Code"
        />
      )}
    </>
  );
