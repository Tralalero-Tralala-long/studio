...
interface BrawlStarsCode {
  code: string;
  reward: string;
  id: string;
}

// Placeholder codes - replace with actual Brawl Stars codes
const brawlStarsCodes: BrawlStarsCode[] = [
  // Add Brawl Stars codes here
];
      ...
      <Card className={`${mode === 'gaming' ? 'bg-card border-primary' : 'bg-card'}`}>
        <CardHeader>
          ...
          <CardDescription className={`${mode === 'gaming' ? 'text-muted-foreground font-rajdhani' : 'text-muted-foreground'} pt-2`}>
            Active codes for Brawl Stars. Redeem them in-game for rewards!
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {brawlStarsCodes.length > 0 ? (
            brawlStarsCodes.map((item) => (
              <Card 
                key={item.id} 
            ...
            <div className="p-6 border border-dashed rounded-lg text-center text-muted-foreground">
              No active Brawl Stars codes found at the moment. Check back soon!
            </div>
          )}
        </CardContent>
      </Card>
