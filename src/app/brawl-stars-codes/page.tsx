...
export default function BrawlStarsCodesPage() {
  ...
  const [codes, setCodes] = useState<PromoExample[]>(initialBrawlStarsCodes);
  ...
  const handleAddCodeSubmit = (formData: { title: string; code: string; expiry?: Date; description: string }) => {
    ...
    toast({
      title: "Code Added!",
      description: `"${newPromo.title}" has been successfully added to Brawl Stars Codes.`,
    });
  };
    ...
    <>
      ...
      {isDeveloperMode && (
        <AddCodeForm
          ...
          formTitle="Add New Brawl Stars Code"
        />
      )}
    </>
  );
